...
// This class simulates a small business domain, to check the usability of our various patterns.
// Here, a user can create multiple counters and increment/decrement them at will.
// A user can also share their counters with other users, which will be able to see them (but not edit or share them).
    ...
    private data class Counter(
        ...
        class Service {
            ...
            sealed interface Failures : Failure {
                ...
                data class NotFound(val id: Id) : CustomFailure(NotFound("Could not find user with ID $id")),
                    Failures,
                    Get,
                    Edit,
                    Share

                data class NotTheOwner(val id: Id) : CustomFailure(Unauthorized("Only the owner can access $id")),
                    Failures,
                    Edit,
                    Share

                object Unauthenticated :
                    CustomFailure(Unauthenticated("Authentication is required to access counters")),
                    Failures,
                    Create,
                    List,
                    Get,
